...
# hyperparameters
batch_size = 16 # in how many batches will be tokens represented in and processed parallely
block_size = 128 # sequence length(how many tokens can be represented in one batch)
max_iterations = 1000 # how many times each batch is processed and the weights are updated
eval_interval = 200 # after how many iterations the model should be evaluated
...
eval_iters = 20 # number of iterations during evaluation(update parameters after each evaluation)
n_embd= 128 # dimension of embedding(i.e dimension with which a token is represented)
n_head = 4 # number of attention layers(multi-head attention)
n_layer = 4
